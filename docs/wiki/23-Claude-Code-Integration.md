...
- API URL: https://your-line-worker.workers.dev
- API Key: (wrangler secretで管理)
- SDK: @line-harness/sdk (packages/sdk/)
- ドキュメント: /path/to/line-harness/docs/wiki/
```
...
```bash
# エンドポイント一覧を確認
curl -s https://your-line-worker.workers.dev/openapi.json | jq '.paths | keys'
```
...
| パス | 内容 |
|------|------|
| `/path/to/line-harness/` | プロジェクトルート |
