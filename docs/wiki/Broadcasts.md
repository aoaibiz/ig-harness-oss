...
一斉配信（ブロードキャスト）は、友だち全員またはタグ/セグメントで絞り込んだ対象にメッセージを一括送信する機能です。下書き保存、予約配信、セグメント配信に対応しています。Instagram API の上限を守るため、バッチ間隔を設けて送信負荷を平準化します。

> **無条件のマス DM ではありません（安全ゲート）**:
>
> 1. **キルスイッチ** — 配信の実行（即時 send も cron の予約配信も）は環境変数 `AUTO_DM_ENABLED` が正確に `'1'` のときのみ可能です（デフォルト OFF・fail-closed）。OFF の間、`POST /api/broadcasts/:id/send` は `403 auto_send_disabled` を返し、予約済み配信も発火しません。
> 2. **24h ウィンドウ** — 送信は受信者ごとに 24h スタンダードウィンドウ（相手が最後にこちらへ DM を送ってから 24 時間以内）を実チェックし、**ウィンドウ外の相手はスキップされます**（失敗ではなくスキップ。Meta 側に拒否させてスパムシグナルを蓄積させないため）。「全員」を対象にしても、実際に送信されるのはウィンドウ内の相手だけです。
> 3. **送信上限** — アカウント毎/受信者毎のローリング上限（デフォルト 100 通/時・300 通/24h・受信者あたり 5 通/24h）を送信前に予約して強制します。上限到達で配信は停止します。
>
> 詳細は README の「安全について」を参照してください。
...
```bash
curl -s "https://your-line-worker.workers.dev/api/broadcasts" \
...
```bash
curl -s "https://your-line-worker.workers.dev/api/broadcasts/BROADCAST_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY" | jq
```
...
```bash
curl -X PUT "https://your-line-worker.workers.dev/api/broadcasts/BROADCAST_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "更新後のタイトル",
...
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/broadcasts/BROADCAST_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://your-line-worker.workers.dev/api/broadcasts/BROADCAST_UUID/send" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
curl -X POST "https://your-line-worker.workers.dev/api/broadcasts/BROADCAST_UUID/send-segment" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "conditions": {
