...
IG Harness は、Instagram ビジネスアカウント向けのオープンソース CRM / マーケティングオートメーションツールです。無料（または低コスト）で自分の Cloudflare 上に self-host して運用できます。

**コンセプト**: AIがInstagram DM運用を安全に操作するための基盤。人間は監視し、AI（Claude Code等）がAPIを通じて操作します。全機能がREST APIとして公開されており、管理画面はデータの可視化専用です。**自動 DM 送信はデフォルトで全て OFF（fail-closed）**です — 有効化には `AUTO_DM_ENABLED='1'` の明示設定が必要です（README「安全について」参照）。
...
| ステップ配信 | シナリオ作成、遅延配信、条件分岐、dm_keyword/comment/manualトリガー。送信は受信者毎の24hウィンドウ内のみ |
| 一斉配信 | 全員/タグ絞り込み、予約配信、バッチ送信。`AUTO_DM_ENABLED='1'` が前提・24hウィンドウ外の相手はスキップ（無条件のマスDMではない） |
| 自動応答 | キーワードマッチ（exact/contains）による自動返信 |
| API遵守 | Instagram API の上限管理、エラー監視、送信停止 |
| アフィリエイト・CV計測 | 流入元トラッキング、コンバージョンポイント定義、成果計測 |
| 負荷平準化 | ジッター、バッチ間隔、レート制限によるAPI上限の遵守 |
...
| コンポーネント | URL |
|---------------|-----|
| API | `https://your-line-worker.workers.dev` |
...
17. **[Instagram API のレート制限と遵守](18-Multi-Account-and-BAN.md)** — API上限、エラー処理、スパム防止
