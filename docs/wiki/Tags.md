...
```bash
curl -s "https://your-line-worker.workers.dev/api/tags" \
  -H "Authorization: Bearer YOUR_API_KEY" | jq
```
...
```bash
# 色指定あり
curl -X POST "https://your-line-worker.workers.dev/api/tags" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/tags/TAG_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
