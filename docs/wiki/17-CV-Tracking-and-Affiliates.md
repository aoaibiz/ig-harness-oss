...
#### CV ポイント一覧取得
```bash
curl -X GET "https://your-line-worker.workers.dev/api/conversions/points" \
...
#### CV ポイント作成
```bash
curl -X POST "https://your-line-worker.workers.dev/api/conversions/points" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
#### CV ポイント削除
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/conversions/points/{id}" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
#### コンバージョン記録
```bash
curl -X POST "https://your-line-worker.workers.dev/api/conversions/track" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
#### CV イベント一覧取得 (フィルタ付き)
```bash
curl -X GET "https://your-line-worker.workers.dev/api/conversions/events?\
conversionPointId=550e8400-...&\
friendId=friend-uuid&\
affiliateCode=partner-a&\
...
#### CV レポート取得 (集計)
```bash
curl -X GET "https://your-line-worker.workers.dev/api/conversions/report?\
...
#### アフィリエイト一覧
```bash
curl -X GET "https://your-line-worker.workers.dev/api/affiliates" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

#### アフィリエイト詳細
```bash
curl -X GET "https://your-line-worker.workers.dev/api/affiliates/{id}" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

#### アフィリエイト作成
```bash
curl -X POST "https://your-line-worker.workers.dev/api/affiliates" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "田中太郎",
...
#### アフィリエイト更新
```bash
curl -X PUT "https://your-line-worker.workers.dev/api/affiliates/{id}" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "田中太郎（ゴールド）",
...
#### アフィリエイト削除
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/affiliates/{id}" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://your-line-worker.workers.dev/api/affiliates/click" \
  -H "Content-Type: application/json" \
  -d '{
    "code": "tanaka-01",
...
#### 個別レポート
```bash
curl -X GET "https://your-line-worker.workers.dev/api/affiliates/{id}/report?\
startDate=2026-03-01&endDate=2026-03-31" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
#### 全体レポート
```bash
curl -X GET "https://your-line-worker.workers.dev/api/affiliates-report?\
startDate=2026-03-01&endDate=2026-03-31" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
### Stripe イベント一覧取得
```bash
curl -X GET "https://your-line-worker.workers.dev/api/integrations/stripe/events?\
...
// => https://your-line-worker.workers.dev/auth/line?ref=instagram&redirect=...
```
