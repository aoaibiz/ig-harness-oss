...
| 変数名 | 説明 |
|--------|------|
| `NEXT_PUBLIC_API_URL` | Workers API の URL（例: `https://your-line-worker.workers.dev`） |
...
# デプロイ後に表示されるURLを控える
# 例: https://your-line-worker.workers.dev
```
...
2. Webhook URL に以下を設定:
   ```
   https://your-line-worker.workers.dev/webhook
   ```
3. 「Use webhook」を有効化
...
```bash
cd apps/web
npx wrangler pages deploy .next --project-name=your-name-admin
```
...
```bash
# 友だちにテキストメッセージを送信
curl -X POST https://your-line-worker.workers.dev/api/friends/{friendId}/messages \
...
```bash
# Cloudflare ダッシュボード → Workers → your-worker → Triggers
