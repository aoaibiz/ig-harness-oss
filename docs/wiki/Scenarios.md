...
processStepDeliveries(db, lineClient)
  ...
  └─ for each friend_scenario:
      ├─ 負荷平準化: sleep(addJitter(50, 200)) ms
...
```bash
curl -s "https://your-line-worker.workers.dev/api/scenarios" \
...
```bash
curl -s "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY" | jq
```
...
```bash
curl -X POST "https://your-line-worker.workers.dev/api/scenarios" \
...
```bash
curl -X PUT "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "更新後の名前",
...
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
# テキストステップ（即時配信）
curl -X POST "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID/steps" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "stepOrder": 1,
...
```bash
curl -X PUT "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID/steps/STEP_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "delayMinutes": 2880,
...
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID/steps/STEP_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://your-line-worker.workers.dev/api/scenarios/SCENARIO_UUID/enroll/FRIEND_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
