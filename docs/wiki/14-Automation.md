...
```bash
curl -X GET "https://your-line-worker.workers.dev/api/automations" \
...
```bash
curl -X GET "https://your-line-worker.workers.dev/api/automations/AUTO_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST "https://your-line-worker.workers.dev/api/automations" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```bash
curl -X PUT "https://your-line-worker.workers.dev/api/automations/AUTO_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "isActive": false,
...
```bash
curl -X DELETE "https://your-line-worker.workers.dev/api/automations/AUTO_UUID" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
# 特定オートメーションのログ
curl -X GET "https://your-line-worker.workers.dev/api/automations/AUTO_UUID/logs?limit=50" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
