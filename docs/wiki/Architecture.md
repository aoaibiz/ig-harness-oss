...
scheduled() — 4つの処理を並列実行:
  ...
  1. processStepDeliveries(db, lineClient)
     ...
     │   ├─ 負荷平準化: addJitter(50, 200)ms のランダム遅延
...
## Instagram APIレート遵守と負荷平準化

IG Harness は Instagram API の上限を超えないよう、送信処理を次の方法で制御します:

1. **ジッター配信**: ステップ配信の `next_delivery_at` に ±5分のランダムオフセット
2. **バッチ間隔**: 一斉配信をバッチ処理し、API呼び出しの集中を抑制
3. **レート制限**: 設定したAPI呼び出し上限に達した場合は次の時間枠まで待機
4. **デプロイ分離**: 認証情報と送信キューをデプロイ単位で分離
